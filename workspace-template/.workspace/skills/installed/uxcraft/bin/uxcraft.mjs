...
const markerStart = '<!-- uxcraft:start -->';
const markerEnd = '<!-- uxcraft:end -->';
...
function copyProjectSkillAssets(root, dryRun) {
  const dest = path.join(root, '.uxcraft');
...
function installProject(root, opts) {
  ...
  console.log(`Installing /uxcraft project trigger into ${root}`);
  console.log(`Package root: ${installedPath}`);

  copyProjectSkillAssets(root, dry);

  if (wanted(opts, 'claude')) writeFile(path.join(root, '.claude', 'commands', 'uxcraft.md'), read('agent-templates/claude/commands/uxcraft.md'), dry);
  ...
  if (wanted(opts, 'windsurf')) {
    writeFile(path.join(root, '.windsurf', 'rules', 'uxcraft.md'), read('agent-templates/windsurf/rules/uxcraft.md'), dry);
    appendMarked(path.join(root, '.windsurfrules'), read('agent-templates/universal/uxcraft-trigger.md'), dry);
  }
  if (wanted(opts, 'antigravity')) writeFile(path.join(root, '.antigravity', 'rules', 'uxcraft.md'), read('agent-templates/universal/uxcraft-trigger.md'), dry);
  if (wanted(opts, 'gemini')) appendMarked(path.join(root, 'GEMINI.md'), read('agent-templates/gemini/GEMINI.append.md'), dry);
  if (wanted(opts, 'cursor')) writeFile(path.join(root, '.cursor', 'rules', 'uxcraft.mdc'), read('agent-templates/cursor/rules/uxcraft.mdc'), dry);

  console.log('Done. Use: /uxcraft <your normal UI/UX request>');
}
...
function installGlobal(opts) {
  const root = path.join(os.homedir(), '.uxcraft');
  console.log(`Installing global UXCraft files into ${root}`);
  copyDir(packageRoot, root, opts.dryRun);
  const claudeTarget = path.join(os.homedir(), '.claude', 'commands', 'uxcraft.md');
  writeFile(claudeTarget, read('agent-templates/claude/commands/uxcraft.md'), opts.dryRun);
...
function uninstallProject(root, opts) {
  const dry = opts.dryRun;
  const files = [
    path.join(root, '.uxcraft'),
    path.join(root, '.claude', 'commands', 'uxcraft.md'),
    path.join(root, '.windsurf', 'rules', 'uxcraft.md'),
    path.join(root, '.antigravity', 'rules', 'uxcraft.md'),
    path.join(root, '.cursor', 'rules', 'uxcraft.mdc'),
  ];
  for (const file of files) {
...
function doctor(opts) {
  ...
  console.log(`uxcraft package root: ${packageRoot}`);
  console.log(`target root: ${root}`);
  console.log(`node: ${process.version}`);
  console.log(`trigger: /uxcraft`);
  const required = ['SKILL.md', 'README.md', 'llms.txt', 'references/ui-ux-complete-checklist.md', 'agent-templates/universal/uxcraft-trigger.md', 'system-prompts/uxcraft-system-add-on.md', 'docs/mcp-server.md', 'bin/uxcraft-mcp.mjs'];
  let ok = true;
...
function help() {
  console.log(`UXCraft\n\nUsage:\n  uxcraft install [--project|--global] [--agents claude,codex,windsurf,antigravity,gemini,cursor,universal] [--dir path] [--dry-run]\n  uxcraft uninstall [--project] [--dir path] [--dry-run]\n  uxcraft doctor [--dir path]\n  uxcraft where\n  uxcraft mcp\n  uxcraft-mcp\n\nAfter install, use: /uxcraft <your normal UI/UX prompt>\nMCP: npx -y --package uxcraft uxcraft-mcp\n\nNote: ui-ux-master is a deprecated alias for uxcraft. Please update to uxcraft.`);
}

const opts = parseArgs(process.argv.slice(2));
if (opts.postinstall) {
  console.log('uxcraft installed. Run `npx uxcraft install --project` inside a repo, then use `/uxcraft ...`.');
  ...
  doctor(opts);
} else if (opts.command === 'mcp' || opts.command === 'uxcraft-mcp') {
  await import('./uxcraft-mcp.mjs');
